...
& $javac -proc:none -encoding UTF-8 --release 17 -cp $classpath -d $out (Join-Path $project 'src/main/java/com/gfl/tarkovscav/client/PillagerAnimationFallback.java') (Join-Path $PSScriptRoot 'PillagerAnimationFallbackTest.java')
if ($LASTEXITCODE -ne 0) { throw 'Pillager animation fallback test compilation failed' }
& $java -cp ($out + ';' + $classpath) com.gfl.tarkovscav.client.PillagerAnimationFallbackTest (Join-Path $project 'src/main/resources/assets/tarkovscav/animations/gunner_pillager.animation.json') (Join-Path $project 'src/main/resources/assets/tarkovscav/animations/scav.animation.json')
if ($LASTEXITCODE -ne 0) { throw 'Pillager animation fallback test failed' }
