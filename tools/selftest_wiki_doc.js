...
check(keyMatches.length === 352,
  `Config.java registers 352 define() call sites (346 before the VANT shield + the 5 [shield] keys`
  + ` + grenades.mob.resupplySearchCooldownTicks)`,
  `found ${keyMatches.length}`);
check(keys.length === 324,
  `those call sites carry 324 distinct key names (the shield's 5 call sites add 4 new names,`
  + ` because "enabled" already exists, and the grenade resupply search cooldown adds 1)`,
  `found ${keys.length}`);
