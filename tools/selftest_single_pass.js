...
// the two `useGeckoModel` branches are chosen inside the family helper - so the invariant is still "exactly
...
check((setup.match(/\.registerEntityRenderer\(/g) || []).length === 3
    ...
    && /useGecko \? GunnerPillagerGeoRenderer::new : GunnerPillagerRenderer::new/.test(setup),
...
check((villagerRenderer.match(/addLayer\(new HeldGunLayer\(/g) || []).length === 1
    ...
    && /class HeldGunLayer[\s\S]{0,300}?extends ItemInHandLayer/.test(villagerRenderer)
    && !/GunInHandGeoLayer|GeoLayer/.test(villagerRenderer),
  'the gunner villager draws its gun with the vanilla item layer (subclassed only for the hideGunWhenIdle'
...
check((read('client/RigSupport.java').match(/setHidden\(true\)/g) || []).length === 2,
  'bone hiding happens in exactly two places, both in the once-per-bake hide pass');
...
check(!/RenderSystem\./.test(renderTypes) && !/RenderSystem\./.test(layer) && !/RenderSystem\./.test(read('client/ScavRenderer.java')),
  'no RenderSystem state is touched by this mod: RenderType setup/clear owns shader, blend, cull and colour mask');
check(/never calls `{@code RenderSystem`}/.test(renderTypes) || /never calls .*RenderSystem/.test(renderTypes),
  'and that is documented where the render types are chosen');

console.log('\nthe forward slide and its axis (user-measured):');
check(/DEFAULT_MOUNT_OFFSET = List\.of\("0", "0", "-0\.7"\)/.test(config),
  'the shipped offset default is the user-measured [0, 0, -0.7]');
