...
public final class Renegade {
    ...
    public static boolean noteFriendlyFire(ServerLevel level, Mob attacker, LivingEntity victim) {
        ...
        if (hits >= Config.FACTION_FRIENDLY_FIRE_HITS_TO_ANGER.get() && victim instanceof Mob mobVictim) {
            // setTarget lives on Mob, not on LivingEntity: a victim that is not a Mob (a player, an armour
            // stand) has no target to set and is simply never angered.
            mobVictim.setTarget(attacker);
    ...
    public static void brand(ServerLevel level, Mob attacker, String reason) {
        ...
        int told = 0;
        for (Mob ally : allies) {
            // "Who is now an enemy of the traitor": the same predicate the target selectors use, so this
            // count cannot drift from the behaviour it describes (and a former ally of the same faction
            // counts, which is the whole point of a betrayal).
            if (ally != attacker && Faction.hostile(ally, attacker)) {
                told++;
            }
        }
    ...
    public static void tickDecay(ServerLevel level, Mob mob) {
        // Nothing about a corpse can decay, and the caller already skips it - this is the belt to that
        // braces, so the rule holds for any future caller too.
